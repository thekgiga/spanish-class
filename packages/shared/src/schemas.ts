...
export const createSlotSchema = z
  ...
  .refine(
    (data) => !data.isPrivate || (data.allowedStudentIds && data.allowedStudentIds.length > 0),
...
export const updateSlotSchema = z
  .object({
    ...
    status: z.enum(['AVAILABLE', 'FULLY_BOOKED', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED']).optional(),
  })
  .refine(
    (data) => {
...
export const bookingsQuerySchema = paginationSchema.extend({
  status: z.enum(['CONFIRMED', 'CANCELLED_BY_STUDENT', 'CANCELLED_BY_PROFESSOR', 'COMPLETED', 'NO_SHOW']).optional(),
