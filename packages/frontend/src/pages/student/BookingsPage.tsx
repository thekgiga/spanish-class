...
export function BookingsPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-4">
      ...
      {/* FEED-001: rate completed lesson */}
      {ratingTarget && (
        <Suspense fallback={null}>
          <RateUserModal
            open={!!ratingTarget}
            onClose={() => setRatingTarget(null)}
            rateeId={(ratingTarget.slot as any).professorId}
            rateeName={
              (ratingTarget.slot as any).professor
                ? `${(ratingTarget.slot as any).professor.firstName} ${(ratingTarget.slot as any).professor.lastName}`
                : t("professor_fallback")
            }
            bookingId={ratingTarget.id}
            onSuccess={() => {
              qc.invalidateQueries({ queryKey: ["student-bookings"] });
              setRatingTarget(null);
              uiToast.success(t("feedback_sent"));
            }}
          />
        </Suspense>
      )}
